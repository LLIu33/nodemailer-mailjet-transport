...
Message.prototype.setHtmlBody = function (htmlBody) {
  ...
  this.HtmlPart = htmlBody;
};

Message.prototype.setTextBody = function (textBody) {
  if (!textBody) { return; }

  this.TextPart = textBody;
};

Message.prototype.setHeaders = function (headers) {
  if (headers.length > 0) {
    this.Headers = headers.reduce((map, headerItem) => {
      map[headerItem.Name] = headerItem.Value;
      return map;
    }, {});
  }
};
  ...
  if (attachments.length > 0) {
    this.Attachments = attachments.map((attachment) => {
      return {
        ...
        Base64Content: attachment.content.toString(),
